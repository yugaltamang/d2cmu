...
const Burst = ({ className = "", color = "hsl(var(--ink))" }: { className?: string; color?: string }) => (
...
const Scribble = ({ className = "", color = "hsl(var(--ink))" }: { className?: string; color?: string }) => (
...
const CTA = () => {
  return (
    ...
    >
      <div className="relative mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-10 text-[hsl(var(--ink))]">
        <div className="flex items-center justify-between gap-3 pb-4 border-b-2 border-[hsl(var(--ink))]">
          ...
          <span className="font-sans text-[11px] uppercase tracking-[0.4em] font-bold opacity-70">
        ...
        <div className="mt-10 sm:mt-14 grid grid-cols-12 gap-6 items-end">
          <div className="col-span-12 lg:col-span-9 relative">
            <h2
              className="font-sans uppercase text-[hsl(var(--ink))] text-[clamp(2.5rem,9vw,7.5rem)] leading-[0.85] tracking-[-0.05em]"
              style={{ fontWeight: 900 }}
            >
              Stop reading.
              <br />
              <span className="relative inline-block">
                <span style={{ background: "hsl(var(--ink))", color: "hsl(var(--lime))", padding: "0 0.12em" }}>
                  Start
                </span>{" "}
                selling
                <Scribble className="absolute -bottom-3 sm:-bottom-5 left-0 w-full h-[24px] sm:h-[36px]" color="hsl(var(--ink))" />
              </span>
              .
            </h2>
            <Burst className="absolute -top-4 right-0 sm:top-2 sm:right-12 h-12 w-12 rotate-12" color="hsl(var(--ink))" />
          </div>
        ...
        <p className="mt-8 max-w-xl text-[hsl(var(--ink))]/80 leading-relaxed text-sm sm:text-base">
          Limited seats. Application-based. Cohort 01 begins{" "}
          <span className="font-bold text-[hsl(var(--ink))]">June 2026</span> at the Masters&apos; Union campus, Gurugram.
        </p>

        <div className="mt-6 flex flex-wrap items-center gap-3">
          <a href="#" className="btn-pill-light group" style={{ background: "hsl(var(--ink))", color: "hsl(var(--lime))", borderColor: "hsl(var(--ink))" }}>
            Start your brand
          ...
          <a href="#" className="btn-pill-ghost" style={{ color: "hsl(var(--ink))", borderColor: "hsl(var(--ink))" }}>
        ...
        <div className="mt-12 grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {details.map(({ icon: Icon, label, value }) => (
            <div
              key={label}
              className="bg-[hsl(var(--paper))] border-2 border-[hsl(var(--ink))] p-4 sm:p-5"
              style={{ boxShadow: "4px 4px 0 hsl(var(--ink))" }}
            >
              <div className="flex items-center justify-between">
                <Icon className="h-5 w-5 text-[hsl(var(--ink))]" strokeWidth={2.25} />
                <span className="h-1.5 w-1.5 rounded-full" style={{ background: "hsl(var(--lime))" }} />
              </div>
              <p className="mt-4 font-sans text-[10px] uppercase tracking-[0.3em] text-[hsl(var(--ink))]/55 font-bold">
                {label}
              </p>
              <p className="mt-2 font-sans uppercase text-[hsl(var(--ink))] text-[15px] sm:text-[17px] leading-tight tracking-[-0.01em]" style={{ fontWeight: 800 }}>
                {value}
              </p>
            </div>
          ))}
